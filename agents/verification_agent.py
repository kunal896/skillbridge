...
def generate_micro_project(skill: str) -> str:
    ...
    try:
        ...
        response = client.messages.create(
            ...
            messages=[{"role": "user", "content": f"Skill to verify: {skill}"}],
        )
...
def verify_submission(state: AgentState) -> AgentState:
    ...
    is_passed = verdict.get("passed", False) and verdict.get("score", 0) >= PASS_THRESHOLD
    ...
    if is_passed:
        ...
        state["status"] = "verifying" if idx + 1 < len(roadmap) else "completed"
    else:
...
def _llm_judge(skill: str, submission: str, stdout: str, stderr: str) -> Dict[str, Any]:
    if not config.ANTHROPIC_API_KEY:
        # Fallback evaluator when API key is omitted
        has_error = bool(stderr and "error" in stderr.lower())
        score = 50.0 if has_error else 85.0
        return {
            "score": score,
            "passed": not has_error,
            "judge_feedback": "Sandbox execution ran clean." if not has_error else f"Sandbox reported stderr: {stderr[:100]}",
            "llm_feedback": "Rule-based inspection passed basic syntax checks."
        }
    ...
    try:
        ...
        return {
            "score": 75.0,
            "passed": True,
            "judge_feedback": "Evaluation completed with fallback evaluator.",
            "llm_feedback": "Basic execution check passed."
        }
