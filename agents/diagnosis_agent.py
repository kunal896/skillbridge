...
def diagnose(state: AgentState) -> AgentState:
    ...
    if state.get("current_skills") and state.get("status") in ("verifying", "awaiting_submission", "completed"):
        logger.info("Diagnosis already performed; skipping re-diagnosis.")
        return state
    ...
    if not llm_client.is_configured():
        logger.warning(
            ...
            "Using rule-based fallback diagnosis.", config.LLM_PROVIDER
        )
        return _fallback_diagnosis(state)

    try:
        raw_text = llm_client.complete(_SYSTEM_PROMPT, user_content, max_tokens=1024)
